 ...
 * @returns {String}
 */
function getHtmlContent2( htmlContent , jQuery, fileArr, urlFunc, scriptAttr ) {
    var $ = jQuery,
        jsArr = [],
        cssArr = [];
        ...
        scriptDomArr.each(function () {
            ...
            if (!util.isExternalUrl(scriptUrl)) {
                for (var i = 0, length = jsArr.length; i < length; i++) {
                    ...
                    if (util.isMatch(scriptUrl, one.filePathInHtml)) {
                        htmlContent = urlFunc( htmlContent , scriptDom, scriptUrl, one, scriptAttr);
        ...
        cssDomArr.each(function () {
            ...
            if (!util.isExternalUrl(cssUrl)) {
                for (var i = 0, length = cssArr.length; i < length; i++) {
                    ...
                    if (util.isMatch(cssUrl, one.filePathInHtml)) {
                        htmlContent = urlFunc( htmlContent, cssDom, cssUrl, one, 'href');
                        break;
                    }
                }
            }
 ...
 * @returns {String}
 */
exports.getHtmlContentInline = function ( htmlContent , jQuery, fileArr, scriptAttr, grunt) {
    return getHtmlContent2(htmlContent , jQuery, fileArr, function ( html , elem, url, item, attrName) {
...
/* inline的方式
        ...
        grunt.log.writeln(' replace ' + elem);
        return html.replace( elem ,  content );
        */

        // Print a success message.

        grunt.log.writeln(' replace to ' + replaceEle);

        return html.replace( elem ,  replaceEle );
...
exports.dealRequireJSConfig = function (grunt, taskTarget, fileArr, type, configUrl, baseUrl) {
    ...
    if (!configObj) {
        ...
        configObj.content = grunt.file.read(configObj.url);
